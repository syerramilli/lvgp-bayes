...
class ExpHalfHorseshoePrior(Prior):
    ...
    the noise variance. The default of 1e-6 works well for most examples, and does not have
    to be tinkered.  The `scale` parameter is more important. The default value for `scale` 
    - 0.01 - works well for deterministic and low-noise situations. A larger value may be need 
    in noisy sitations.
